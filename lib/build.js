...
function scanEntry(pwd, branches) {
    ...
    readFilesOfDir(startPath, function (path) {
        var name = prefix + path.replace(startPath, '').replace(/^[^\w]/, '').replace(/\.entry\.(jsx|scss|less|js)$/, '');
    ...
    // for css
    startPath = Path.join(pwd, './src/static/css');
    readFilesOfDir(startPath, function (path) {
        var name = prefix + path.replace(startPath, '').replace(/^[^\w]/, '').replace(/\.entry\.(jsx|scss|less|js)$/, '');
        entry[name] = entry[name] || [];
        entry[name].push('.' + connection + Path.relative(pwd, path));
    ...
    // for less
    startPath = Path.join(pwd, './src/static/less');
    readFilesOfDir(startPath, function (path) {
    ...
    console.log(chalk.cyan(JSON.stringify(newEntry, '', 2)));
}
...
function readFilesOfDir(dir, cb) {
    if (fs.existsSync(dir)) {
        ...
        dirs.forEach((subDir) => {
            ...
            if (isFile) {
                if (/\.entry\.(jsx|scss|less|js)$/.test(useDri)) {
                    cb(useDri);
                }
            } else {
